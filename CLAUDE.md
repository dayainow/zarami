...
## 🤖 5. 클로드(Claude) - 제미나이(Gemini) 이중 작업(Dual-Agent) 협업 룰

본 레포지토리는 Claude와 Gemini가 역할을 분담하여 효율적으로 기능을 구현하기 위한 이중 작업 룰을 따릅니다.
두 AI 에이전트는 충돌(Conflict)을 방지하기 위해 다음 원칙을 의무적으로 준수합니다.

### 5.1 브랜치 네이밍 및 격리 원칙
- **작업 브랜치 강제:** 어떠한 경우에도 `main` 브랜치에 직접 커밋하지 않습니다.
- **브랜치 네이밍 규약:**
  - Claude 담당 작업: `feature/claude/[KAN-ID]-[기능요약]`
  - Gemini 담당 작업: `feature/gemini/[KAN-ID]-[기능요약]`
- **상호 불가침:** 각 에이전트는 상대방의 Prefix가 붙은 브랜치의 코드를 임의로 변경하지 않습니다. 공통 설정 파일(`package.json` 등) 변경 시 사용자에게 고지하거나 PR을 통해 조율합니다.

### 5.2 핸드오프(Handoff) 프로토콜
- 작업이 끝나고 리모트에 Push/PR을 올리면, `TASK_MASTER.md` 파일에 완료 여부를 체크(`[x]`)하고 진행한 브랜치명을 간략히 남깁니다.
- 새로운 티켓을 시작하기 전에는 항상 `TASK_MASTER.md`를 먼저 스캔하여 상대 AI가 방금 마친 작업이나 진행 중인 작업을 파악(Context Sync)합니다.
- 하나의 KAN 티켓은 반드시 한 명의 AI 에이전트가 전담하여 끝까지 마무리합니다.
