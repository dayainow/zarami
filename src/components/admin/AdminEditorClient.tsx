...
export function AdminEditorClient() {
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-slate-50 px-5 text-slate-950 transition-colors duration-300 dark:bg-slate-950 dark:text-white">
        <div className="max-w-sm rounded-xl border border-white/70 bg-white/75 p-6 text-center shadow-xl shadow-slate-900/10 backdrop-blur-2xl dark:border-white/10 dark:bg-white/[0.04] dark:shadow-black/20">
          <h1 className="text-lg font-bold text-slate-950 dark:text-white">어드민 로그인이 필요합니다</h1>
          ...
          {authStatus === "signed-out" ? (
            <div className="mt-6 flex flex-col items-center">
              {emailSent ? (
                <p className="text-sm font-bold text-emerald-600 dark:text-emerald-400">이메일로 로그인 링크를 보냈습니다!</p>
              ) : (
  ...
  return (
    <main className="flex h-screen w-full overflow-hidden bg-slate-50 text-slate-950 transition-colors duration-300 dark:bg-slate-950 dark:text-white">
      ...
      <aside className="flex w-[360px] shrink-0 flex-col border-l border-white/70 bg-white/72 p-5 shadow-2xl shadow-slate-900/10 backdrop-blur-2xl dark:border-white/10 dark:bg-slate-900/72 dark:shadow-black/30">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-emerald-600 dark:text-emerald-300">노드 속성</h2>
          <button
            type="button"
            onClick={() => void handleExportJson()}
            className="shrink-0 rounded-lg bg-sky-500 px-3 py-1.5 text-xs font-bold text-white shadow-lg shadow-sky-500/20 transition hover:bg-sky-400 dark:bg-sky-400 dark:text-slate-950 dark:hover:bg-sky-300"
          >
        ...
        {!data ? (
          <p className="mt-6 text-sm text-slate-600 dark:text-slate-400">캔버스에서 노드를 클릭해 편집하세요.</p>
        ) : (
          <div className="mt-4 flex-1 space-y-4 overflow-y-auto pr-1">
            ...
            <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400">
              ...
              <input
                ...
                className="mt-1 w-full rounded-lg border border-slate-200/80 bg-white/70 px-3 py-2 text-sm text-slate-950 shadow-sm backdrop-blur-xl transition focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 dark:border-white/10 dark:bg-white/5 dark:text-white"
              />
              <span className="mt-1 block font-normal normal-case text-slate-500 dark:text-slate-500">
            ...
            <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400">
              ...
              <textarea
                ...
                rows={6}
                className="mt-1 w-full rounded-lg border border-slate-200/80 bg-white/70 px-3 py-2 font-mono text-xs text-slate-950 shadow-sm backdrop-blur-xl transition focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 dark:border-white/10 dark:bg-white/5 dark:text-white"
              />
            </label>
